				...
				<slot name="up" v-if="hasData" :up="up">
					...
					<div class="pull-up-tips" v-show="up != false">
						<div class="tips-content">
							...
							<slot name="no-more" v-bind:isShow="up.state === 3">
...
export default {
	...
	computed: {
		timeDifferenceText() {
			...
			let timeDifference =
				(this.startPullTime - this.lastQueryTime) / 1000;
	...
	methods: {
		...
		// 初始化dom
		initDom() {
			this.downElHeight = this.$refs["scroller"].querySelector(
				".pull-down-tips"
			)
				? this.$refs["scroller"].querySelector(".pull-down-tips")
						.offsetHeight
				: 0;
			this.upElHeight = this.$refs["scroller"].querySelector(
				".pull-up-tips"
			)
				? this.$refs["scroller"].querySelector(".pull-up-tips")
						.offsetHeight
				: 0;
			this.iScrollOptions.startY = -this.downElHeight;
			// 最小高度要加1，防止数据少一屏时IScroller无法滑动问题
			this.scrollerMinHeight =
				this.$refs["scroller"].offsetHeight -
				(this.iScrollOptions.startY < 0
					? this.iScrollOptions.startY
					: -1);
			this.$refs["scroller"].querySelector(
				".list-pull-loading-scroller"
			).style.minHeight = this.scrollerMinHeight + "px";
		},
		// 初始化iScroll组件
		initIScroll() {
			...
			this.myScroll.on("scroll", function () {
				...
				if (
					_this.down &&
					!_this.isLoading &&
					y + _this.downElHeight >= _this.down.offset
				) {
					// 下拉
					...
					!_this.isLoading &&
					_this.up.state != 3 &&
					maxY >= _this.up.offset - _this.upElHeight
				) {
					// 上拉
			...
			this.myScroll.on("scrollEnd", function () {
				...
				if (_this.endPullTime - _this.startPullTime < 0) {
					...
					if (this.y > -_this.downElHeight) {
						...
						this.maxScrollY + _this.upElHeight > this.y &&
						this.maxScrollY < -this.downElHeight - this.upElHeight
					) {
						_this.resizeIScrollPosition(
							this.maxScrollY + _this.upElHeight
						);
					}
					return;
				}
				...
				if (_this.down.state == 1) {
					...
					this.maxScrollY <= this.y &&
					this.maxScrollY + _this.upElHeight > this.y &&
					this.maxScrollY < -_this.downElHeight - _this.upElHeight
				) {
					// 上拉复位
					_this.resizeIScrollPosition(
						this.maxScrollY + _this.upElHeight
		...
		initMaxScroller(maxTimes) {
			...
			if (
				...
				this.up &&
				this.up.state !== 3 &&
				this.myScroll.maxScrollY >= -this.downElHeight - this.upElHeight
			) {
				this.updateUpState(2);
		...
		initIScrollCacheData(y = 0, upState = 0, hasData = true) {
			...
			let scrollTo = function (scrollY) {
				if (scrollY > -_this.downElHeight) {
					...
					_this.myScroll.maxScrollY <
						-_this.downElHeight - _this.upElHeight
				) {
					// 最大滚动高度
					scrollY = _this.myScroll.maxScrollY + _this.upElHeight;
				}
			...
			return new Promise((resolve, reject) => {
				...
				if (this.imgResize && this.$refs["scroller"]) {
					let _this = this,
						_timer_id = -1,
						_resolve_timer_id = -1,
						_el_length = this.$refs["scroller"].querySelectorAll(
							"img[lazy='loading']"
						).length;
					if (_el_length === 0) {
						// 没有懒加载的图片
					...
					Array.from(
						this.$refs["scroller"].querySelectorAll(
							"img[lazy='loading']"
						)
					).forEach((imgEl, index) => {
		...
		query(type) {
			...
			this.isLoading = true;
			return this.api(this.parameters, type === 1)
				.then((listLength) => {
					this.isLoading = false;
					this.hasData = !(type != 1 && listLength === 0);
					if (listLength < this.parameters.maxResultCount)
						this.updateUpState(3);
		...
		upLoaddingEvent() {
			...
			this.query(1).then(() => {
				...
				if (this.myScroll.maxScrollY > _myScrollY) {
					this.resizeIScrollPosition(
						this.myScroll.maxScrollY + this.upElHeight
					);
				}
			});
		},
